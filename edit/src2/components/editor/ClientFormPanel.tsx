...
export const ClientFormPanel: React.FC = () => {
  ...
  const [ogDescription, setOgDescription] = useState(
    "Kami mengundang Anda untuk merayakan hari kebahagiaan pernikahan kami.",
  );
  ...
  const activeSectionTypeInfo = SECTION_TYPES_LIST.find(
    (s) => s.type === activeSection?.sectionType,
  );

  const textLayers =
    activeSection?.contentJson.layers.filter((l) => l.type === "text") || [];
        ...
        <div className="flex items-center gap-2.5 mb-2">
          ...
          <div>
            ...
            <p className="text-[11px] text-slate-500">
              Personalisasi teks isi & Open Graph WhatsApp
            </p>
          </div>
        </div>
      ...
      {activeTab === "content" ? (
        <>
          ...
          <div className="p-3 border-b border-slate-100 bg-slate-50/50">
            ...
            >
              {sections.map((sec) => {
                const secInfo = SECTION_TYPES_LIST.find(
                  (s) => s.type === sec.sectionType,
                );
                return (
                  <option key={sec.id} value={sec.id}>
                    {sec.sectionOrder}.{" "}
                    {secInfo ? secInfo.label.split(". ")[1] : sec.sectionType}
                  </option>
                );
              })}
            </select>
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            {textLayers.length === 0 ? (
              ...
              textLayers.map((layer) => {
                ...
                const currentValue =
                  overrides.textOverrides[overrideKey] ?? layer.content;
                  ...
                  >
                    ...
                    {layer.content.length > 40 ? (
                      <textarea
                        ...
                        value={currentValue}
                        onChange={(e) =>
                          setOverride(overrideKey, e.target.value, "text")
                        }
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          <div className="p-3 bg-blue-50/70 border border-blue-100 rounded-2xl text-xs text-blue-900">
            <span className="font-bold block mb-1 flex items-center gap-1">
              <Share2 className="w-3.5 h-3.5 text-blue-600" /> WhatsApp Link
              Preview
            </span>
            Atur thumbnail dan judul sosial media ketika link dikirim via
            WhatsApp.
          </div>
