...
import { CANVAS_WIDTH, CANVAS_HEIGHT, ElementLayer, AnimationType } from '@/types/builder';
import {
  Lock,
  RotateCcw,
  Trash2,
...
export const Canvas: React.FC = () => {
  ...
  const activeSectionIndex = sections.findIndex((sec) => sec.id === activeSectionId);
    ...
    switch (type) {
      ...
      case 'floating-sway':
        return {
          ...
          animate: { y: [0, -10, 0, 10, 0], rotate: [0, 2, 0, -2, 0], opacity: 1 },
          transition: { repeat: Infinity, duration: 4, ease: 'easeInOut' },
        };
      case 'gravity-drop':
        return {
          ...
          transition: { type: 'spring', bounce: 0.6, damping: 9, stiffness: 140 },
        };
      case 'curved-spiral':
        return {
        ...
        >
          {activeSection?.contentJson.backgroundOverlay && (
            <div
              ...
              style={{ backgroundColor: activeSection.contentJson.backgroundOverlay }}
            />
          )}
                  ...
                  >
                    ...
                    {layer.type === 'shape' && (
                      <div
                        ...
                        style={{
                          backgroundColor: layer.style.backgroundColor || '#2563EB',
                          borderRadius: layer.style.borderRadius
                            ? `${layer.style.borderRadius}px`
                            : '8px',
                        }}
                    ...
                    {isSelected && (
                      <div className="absolute -top-7 left-0 bg-blue-600 text-white text-[10px] px-2 py-0.5 rounded-md font-mono font-bold tracking-tight flex items-center gap-1 shadow-md pointer-events-none z-50 whitespace-nowrap">
                        ...
                        {layer.isLocked && <Lock className="w-2.5 h-2.5 text-amber-300" />}
                      </div>
                    )}
                  </motion.div>
                </Rnd>
      ...
      <div className="w-full max-w-xl bg-white border border-slate-200 shadow-md rounded-full px-5 py-2 mt-3 flex items-center justify-between text-xs text-slate-700 font-semibold">
        ...
        <div className="flex items-center gap-3">
          <span className="text-[11px] text-slate-500 font-mono">
            Section {activeSection ? activeSection.sectionOrder : 1}/{sections.length}
          </span>
        </div>
      </div>
    </div>
