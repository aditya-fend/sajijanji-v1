  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.success && res.redirectUrl) {
        ...
        localStorage.setItem(`project_${res.projectId}`, JSON.stringify(newProject));
      ...
      setErrorMsg(err.message || 'An error occurred during project scaffolding');
      setIsSubmitting(false);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-between p-6 select-none font-sans">
      ...
      <header className="max-w-4xl w-full mx-auto flex items-center justify-between py-4 border-b border-slate-200">
        <div className="flex items-center gap-2.5">
          ...
          <span className="font-bold text-base tracking-wide text-slate-900">Sajijanji Studio</span>
        </div>
      ...
      <main className="max-w-xl w-full mx-auto my-auto bg-white border border-slate-200/90 rounded-3xl p-8 shadow-xl">
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Inisialisasi Proyek Undangan Baru</h1>
          <p className="text-xs text-slate-500">
            Masukkan nama kedua mempelai untuk membuat URL slug khusus & metadata Open Graph.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            >
              <option value="demo-template-1">Tema 1: Soft Pastel Floral (12 Sections)</option>
              <option value="demo-template-2">Tema 2: Minimalist Studio Luxury</option>
              <option value="demo-template-3">Tema 3: Vintage Botanical Garden</option>
              <option value="demo-template-4">Tema 4: Royal Blue Gold Romance</option>
            </select>
          </div>
          ...
          {groomName && brideName && (
            <div className="p-3.5 bg-blue-50/70 border border-blue-100 rounded-2xl text-xs space-y-1">
              <span className="text-blue-900 font-bold block">Generated Public Link URL:</span>
